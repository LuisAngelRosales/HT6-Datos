...
public class _TreeMap_<K,V> implements InterMap<K,V>{
	
	protected TreeMap<K, V> cartas;
	
	public _TreeMap_() {
		cartas = new TreeMap<K,V>();
	} 
	...
	public int cantidadCartas(int type) {
		...
		int cantidad = 0;
		for(Map.Entry m:cartas.entrySet()) {
			if (m.getValue().equals("SPELL")) {
				contSpell += 1;
				}
		}
		for(Map.Entry m:cartas.entrySet()) {
			if (m.getValue().equals("TRICK")) {
				contTrick +=1;
				}
		}
		
		for(Map.Entry m:cartas.entrySet()) {
			if (m.getValue().equals("MONSTER")) {
				contMonster +=1;
				}
		}
